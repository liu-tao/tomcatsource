...
public class TestStandardContext extends TomcatBaseTest {
    ...
    public void testBug46243() throws Exception {
        ...
        assertEquals(Bug46243Filter.class.getName() + HelloWorldServlet.RESPONSE_TEXT,
    ...
    public void testBug49711() {
        ...
        assertEquals("Incorrect response for non-configured casual multipart request", "parts=0", // multipart request should be ignored
            client.getResponseBody());
        ...
        assertEquals("Incorrect response for configured casual multipart request", "parts=1",
            client.getResponseBody());
    ...
    private static class Bug49711Servlet extends HttpServlet {
        ...
        protected void service(HttpServletRequest req, HttpServletResponse resp)
            ...
            out.println("parts="
                        + (null == req.getParts() ? "null" : Integer.valueOf(req.getParts().size())));
        }
    }
        ...
        private Exception doRequest(String uri, boolean allowCasualMultipart,
            ...
            try {
                ...
                if (makeMultipartRequest) {
                    ...
                    request = new String[] { "POST http://localhost:" + getPort() + uri
                                             + " HTTP/1.1" + CRLF + "Host: localhost" + CRLF
                                             + "Connection: close" + CRLF
                                             + "Content-Type: multipart/form-data; boundary="
                                             + boundary + CRLF + "Content-Length: "
                                             + content.length() + CRLF + CRLF + content + CRLF };
                } else {
                    request = new String[] { "GET http://localhost:" + getPort() + uri
                                             + " HTTP/1.1" + CRLF + "Host: localhost" + CRLF
    ...
    public void testFlagFailCtxIfServletStartFails() throws Exception {
        ...
        StandardContext context = (StandardContext) tomcat
            .addContext("", docBase.getAbsolutePath());
